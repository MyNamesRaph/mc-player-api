...
export const handler: Handlers<Player[] | null> = {
  async GET(ctx) {
    const resp = await fetch(
      `http://localhost:8000/api/players/` + ctx.params.name,
    );
    if (resp.status === 404) {
      ...
      <div class="bg-stone-dark flex-1 text-white">
        <div class="flex justify-center h-full">
          <div class="block p-6 rounded-lg shadow-lg bg-white w-full m-10">
            <h5 class="text-gray-900 text-3xl leading-tight mb-2">
              {player.name}
            </h5>
            <div class="flex flex-wrap">
              <div class="">
                {player.textures.SKIN !== undefined &&
                  player.textures.SKIN !== null &&
